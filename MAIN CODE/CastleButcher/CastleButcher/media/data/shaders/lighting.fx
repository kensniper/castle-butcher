...
void ps_diff(in v2p IN,out p2s OUT)
{
	...
	float diff=saturate(dot(IN.LightVec,normal))+0.1;
	OUT.color.rgb=color.rgb*diff;
	OUT.color.a=color.a;
}
...
technique Diffuse
{
	pass p0
	{
		vertexshader=compile vs_3_0 vs_diff();
		pixelshader=compile ps_3_0 ps_diff();
	}
}
technique DiffuseNormal
{
	...
	{
		vertexshader=compile vs_3_0 vs_diff();
		pixelshader=compile ps_3_0 ps_diff_bump();
	}
}
technique DiffuseNormalSpecular
{
	pass p0
	{
		vertexshader=compile vs_3_0 vs_diffspec();
		pixelshader=compile ps_3_0 ps_diffspec_bump();
	}
}
...
technique EmissiveDiffuseNormalSpecular
{
	...
	{
		vertexshader=compile vs_3_0 vs_diffspec();
		pixelshader=compile ps_3_0 ps_emisdiffspec_bump();
	}
}
